...
export default {
  ...
  aliases: ['selesai'],
  category: 'store',
  description: 'Tandai order selesai & kirim notifikasi',
  usage: 'done @tag keterangan',
  permission: 'admin',
  ...
  run: async ({ sock, m, text, args }) => {
    ...
    if (!target) {
      return m.reply(
        ...
        `📌 *Cara pakai:*\n` +
        `done @tag keterangan\n` +
        `done 08xxxx keterangan\n\n` +
        `📝 *Contoh:*\n` +
        `done @user Akun Netflix sudah dikirim\n` +
        `done 081234567890 Akun Spotify sudah aktif\n\n` +
    ...
    if (store.doneTemplate) {
      ...
      message += `\n───···\n`
      message += `Terima kasih sudah berbelanja di *${config.storeName}*!`
    }
